...
namespace WebEngine::Resources::Levels
{
    ...
    DefaultLevel::~DefaultLevel()
    {
        UI::MaterialSelector::MaterialUpdateEvent.removeListener( m_materialUpdateListenerID );

        if ( m_directionalLight )
            m_directionalLight = nullptr;

        for ( PointLightPtr& ptr : m_pointLights )
            ptr = nullptr;
        ...
        if ( m_cameraMovement )
            m_cameraMovement = nullptr;

        if ( m_camera )
            m_camera = nullptr;
    }

    void DefaultLevel::initDefault()
    {
        ...
        m_model = parser.loadModel( "assets/models/nanosuit/nanosuit.obj", parserFlags );
        ...
        m_materialUpdateListenerID = UI::MaterialSelector::MaterialUpdateEvent.addListener( 
            std::bind( &DefaultLevel::updateMaterial, this, std::placeholders::_1, std::placeholders::_2 ) );
    }

    void DefaultLevel::initLights()
    {
        m_directionalLight = std::make_unique< Lights::DirectionalLight >();
        m_directionalLight->setDirection( glm::vec3( 0.0f, 0.0f, -0.1f ) );
        m_directionalLight->setDiffuse( glm::vec3( 0.7f ) );
        m_directionalLight->setSpecular( glm::vec3( 1.0f ) );
        m_directionalLight->setColor( glm::vec3( 1.0f, 1.0f, 0.0f ) );
        ...
        pointLight1->setAttenuation( glm::vec3( 1.0f, 0.045f, 0.0075f ) );
        pointLight1->setColor( glm::vec3( 1.0f, 0.0f, 0.0f ) );
        pointLight1->setPosition( glm::vec3( 1.0f, -5.0, -17.0f ) );
        ...
        pointLight2->setAttenuation( glm::vec3( 1.0f, 0.045f, 0.0075f ) );
        pointLight2->setColor( glm::vec3( 0.0f, 1.0f, 0.0f ) );
        pointLight2->setPosition( glm::vec3( -2.0f, 1.0, -17.0f ) );
        ...
        pointLight3->setAttenuation( glm::vec3( 1.0f, 0.045f, 0.0075f ) );
        pointLight3->setColor( glm::vec3( 0.0f, 0.0f, 1.0f ) );
        pointLight3->setPosition( glm::vec3( 1.0f, 3.0, -18.0f ) );
        m_pointLights[2] = std::move( pointLight3 );
    }

    void DefaultLevel::initMaterials()
    {
    ...
    void DefaultLevel::updateMaterial( std::string mesh, std::string material )
    {
        Resources::Geometry::Mesh& meshRef = m_model->getMesh( mesh );
        meshRef.setMaterial( m_materials[ material ] );
    }

    void DefaultLevel::draw( float dt ) const
    {
